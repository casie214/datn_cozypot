          ...
          <form @submit.prevent="handleSubmit" class="modal-body">
            <div class="info-grid">
              <div class="form-group">
                ...
                <select v-model="formData.idDotKhuyenMai" required>
                  <option value="" disabled selected>Chọn đợt khuyến mãi</option>
              ...
              <div class="form-group">
                ...
                <input v-model="formData.codeGiamGia" type="text" placeholder="VD: VOUCHER2024" required class="voucher-input">
              </div>
            </div>

            <div class="form-group">
              <label>Tên phiếu giảm giá *</label>
              <input v-model="formData.tenPhieuGiamGia" type="text" placeholder="Nhập tên phiếu" required>
            </div>
            ...
            <div class="info-grid">
              <div class="form-group">
                ...
                <input v-model.number="formData.giaTriGiam" type="number" placeholder="0" required>
              </div>
              <div class="form-group">
                <label>Giảm tối đa *</label>
                <input v-model.number="formData.giaTriGiamToiDa" type="number" placeholder="0" required>
              </div>
            </div>
            ...
            <div class="info-grid">
              <div class="form-group">
                ...
                <input v-model.number="formData.donHangToiThieu" type="number" placeholder="0" required>
              </div>
              <div class="form-group">
                <label>Số lượng phát hành *</label>
                <input v-model.number="formData.soLuongPhatHanh" type="number" placeholder="0" required>
              </div>
            </div>
...
const props = defineProps({
  ...
  danhSachDotKM: {
    type: Array,
    default: () => [
      { id: 1, ten: 'Khuyến mãi hè 2026' },
      { id: 2, ten: 'Siêu sale Tết' }
    ]
  }
});
...
.form-group input, .form-group select {
  ...
  background-color: #fcfcfc; color: #333; font-size: 14px; outline: none; transition: 0.2s;
}
.form-group input:focus, .form-group select:focus { border-color: #8B0000; background-color: #fff; }
...
/* Footer Buttons Gradient */
.modal-footer-btns { display: flex; gap: 20px; margin-top: 35px; }
.btn-huy { flex: 1; padding: 12px; border: none; border-radius: 6px; background: #e8e8e8; color: #666; font-weight: bold; cursor: pointer; }
.btn-xac-nhan { flex: 1; padding: 12px; border: none; border-radius: 6px; background: linear-gradient(to right, #6a0000, #d32f2f); color: white; font-weight: bold; cursor: pointer; }


.btn-xac-nhan:hover { filter: brightness(1.1); }
.btn-huy:hover { filter: brightness(0.95); }
