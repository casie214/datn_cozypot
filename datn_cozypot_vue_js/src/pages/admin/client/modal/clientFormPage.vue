...
const handleSave = async () => {
  // 1. Validate FE (lúc này tenDangNhap/matKhauDangNhap vẫn đang trống - ok)
  ...
  // 2. TỰ ĐỘNG GÁN GIÁ TRỊ NGẦM (Chỉ khi thêm mới)
  // Bước này cực kỳ quan trọng: Phải gán VÀO formData thì preparePayload mới lấy được
  if (!clientId.value) {
    // Luôn ưu tiên dùng SĐT làm Username để tránh lỗi độ dài Backend (< 5 ký tự)
    formData.tenDangNhap = formData.soDienThoai;
    
    // Luôn sinh mật khẩu mới nếu chưa có
    if (!formData.matKhauDangNhap) {
      formData.matKhauDangNhap = generateRandomPassword(10);
    }
  }

  // 3. Hiển thị xác nhận (Lúc này không hiện User/Pass cho Admin xem nữa)
  const result = await Swal.fire({
    ...swalConfig,
    title: clientId.value ? 'Cập nhật khách hàng?' : 'Xác nhận thêm mới?',
    text: clientId.value 
      ? 'Bạn có chắc chắn muốn lưu các thay đổi?' 
      : 'Hệ thống sẽ tự động tạo tài khoản và gửi thông tin qua Email khách hàng.',
    icon: 'question',
    showCancelButton: true,
  ...
  try {
    ...
    // Debug để bạn tự kiểm tra trong F12 (Admin không thấy cái này)
    console.log("Dữ liệu gửi lên để Backend gửi Mail:", {
       user: payload.get('tenDangNhap'),
       pass: payload.get('matKhauDangNhap')
    });

    await submitClient(payload);

    toast.success("Thao tác thành công! Email đã được gửi.");
    router.push('/admin/client');

  } catch (e) {
    console.error("Chi tiết lỗi:", e.response?.data);
    toast.error("Lỗi dữ liệu: " + (e.response?.data?.message || "Vui lòng kiểm tra lại"));
  } finally {
    loading.value = false;
  }
};
