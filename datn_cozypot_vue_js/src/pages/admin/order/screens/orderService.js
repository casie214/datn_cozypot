...
export const BeSearchHoaDon = async (key, trangThai, tuNgayTao, denNgayTao, tuNgayDat, denNgayDat, page = 0, size = 5,) => {
  try {
    ...
    if (tuNgayTao) params.tuNgayTao = tuNgayTao;
    if (denNgayTao) params.denNgayTao = denNgayTao;

    if (tuNgayDat) params.tuNgayDat = tuNgayDat;
    if (denNgayDat) params.denNgayDat = denNgayDat;
...
export const BeGetChiTietHoaDon = async (idHoaDon) => {
  try {
    const response = await axiosClient.get(
      `${PREFIX}/chi-tiet-hoa-don/${idHoaDon}`,
    );
    return response.data;
  } catch (error) {
    throw error;
...
export const BeUpdateMonDaLen = async (idChiTiet) => {
  try {
    await axiosClient.put(
      `${PREFIX}/chi-tiet-hoa-don/cap-nhat-da-len/${idChiTiet}`,
    );
    return true;
  } catch (error) {
    throw error;
...
export const BeUpdateTatCaDaLen = async (idHoaDon) => {
  try {
    await axiosClient.put(
      `${PREFIX}/chi-tiet-hoa-don/cap-nhat-tat-ca-da-len/${idHoaDon}`,
    );
    return true;
  } catch (error) {
    throw error;
