...
const promotionService = {
    ...
    getActiveMonAns: () => axios.get(`${API_MON_AN}/active`).then(res => res.data),
    // 3. Thêm mới
    create: (data) => axios.post(`${API_URL}/create`, data).then(res => res.data),

    // 4. Cập nhật
    // Trong promotionService.js
    update: (id, data) => {
        // Đảm bảo data gửi đi có cấu trúc phẳng hoặc đúng như API yêu cầu
        return axios.put(`${API_URL}/update/${id}`, data).then(res => res.data);
    },
    ...
    toggleStatus: async (id, currentStatus) => {
        ...
        return axios.put(`${API_URL}/update/${id}`, currentData);
    }
};
