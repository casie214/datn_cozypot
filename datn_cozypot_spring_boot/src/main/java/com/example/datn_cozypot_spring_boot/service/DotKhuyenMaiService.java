...
public class DotKhuyenMaiService {
    ...
    public List<DotKhuyenMaiDTO> search(String keyword, Integer status, Integer type) {

        String searchKey = (keyword == null || keyword.trim().isEmpty())
                ? null
                : keyword.trim();

        List<DotKhuyenMai> listEntity =
                dotKhuyenMaiRepo.searchFilter(searchKey, status, type);
